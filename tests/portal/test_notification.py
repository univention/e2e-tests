...
def test_two_notifications(
    ...
    notification = (
        home_page_logged_in.popup_notification_container.notification(0)
    )
    ...
    assert url_normalize(expected_url) == url_normalize(
        actual_url
    ), (f"Wrong link in notification pop up. Expected: {expected_url}, "
        f"actual: {actual_url}")
    ...
    ), (f"Wrong link target in notification pop up. "
    ...
    ), (f"Wrong link text in notification pop up."
        f" Expected: {expected_link_text}, actual: {actual_link_text}")

    expect(notification.title).to_have_text(
        f"{notification_json_data['severity'].capitalize()}:"
        f" {notification_json_data['title']}",
    )
    expect(
        notification.details
    ).to_have_text(notification_json_data["details"])

    response = requests.post(
        send_notification_endpoint,
        json=notification_json_data_different_details
    )
    assert response.ok, \
        f"Got status {response.status_code} while sending notification"

    home_page_logged_in.reveal_area(
        home_page_logged_in.notification_drawer,
        home_page_logged_in.header.bell_icon
    )
    expect(
        home_page_logged_in.notification_drawer.notifications
    ).to_have_count(2)
    first_notification = (
        home_page_logged_in.notification_drawer.notification(0)
    )
    ...
    second_notification = (
        home_page_logged_in.notification_drawer.notification(1)
    )
    expect(second_notification).to_be_visible()
    expect(
        second_notification.details
    ).to_have_text(notification_json_data["details"])
