...
@pytest.fixture()
def dummy_user_home(
        navigate_to_home_page_logged_in: Page,
        username,
        password
    ...
    # TODO: This step is necessary, because when using
    #  a UCS VM vs. a SouvAP env, the start page after
    #  login is not /umc.
...
def test_non_admin_can_change_password(dummy_user_home: Page):
    ...
    change_password_page.change_password(
        DUMMY_USER_PASSWORD_1,
        DUMMY_USER_PASSWORD_2
    )
    ...
    expect(
        dummy_user_home_logged_in.right_side_menu.logout_button
    ).to_be_visible()
