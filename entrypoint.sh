...
if [ -z "$PYTEST_ADDOPTS" ]; then
    ...
    if [ -n "$NAMESPACE" ]; then
        export PYTEST_ADDOPTS="-m acceptance_environment --admin-username default.admin --admin-password $ADMIN_PASSWORD --username default.user --password $USER_PASSWORD --udm-admin-username cn=admin --udm-admin-password $UDM_ADMIN_PASSWORD --num-ip-block 7 --release-duration 10 --portal-base-url https://portal.$NAMESPACE.gaia.open-desk.cloud/univention/portal/"
    # else if PORTAL_BASE_URL
    elif [ -n "PORTAL_BASE_URL" ]; then
        export PYTEST_ADDOPTS="-m acceptance_environment --admin-username default.admin --admin-password $ADMIN_PASSWORD --username default.user --password $USER_PASSWORD --udm-admin-username cn=admin --udm-admin-password $UDM_ADMIN_PASSWORD --num-ip-block 7 --release-duration 10 --portal-base-url $PORTAL_BASE_URL"
    else
