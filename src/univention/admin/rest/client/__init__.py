...
from typing import (Any, Callable, Dict, Iterator, List, Optional, Text, Type,
                    Union)
...
class Session:
    ...
    def make_request(
        self, method, uri, data=None, expect_json=False, allow_redirects=True, **headers
    ):
        ...
        def doit():
            # type: () -> Response
            try:
                response = self.get_method(method)(
                    uri,
                    params=params,
                    json=json,
        ...
        for _i in range(5):
            try:
                ...
                ServiceUnavailable
            ) as exc:  # TODO: same for ConnectionError? python-request does it itself.
        ...
        if response.status_code >= 399:
            ...
            try:
                ...
                if isinstance(json, dict):
                    ...
                    try:
                        error_details["error"] = list(
                            self.resolve_relations(json, "udm:error")
                        )
                    except _NoRelation:
                        pass
                    if error_details:
    ...
    def get_relations(self, entry, relation, name=None, template=None):
        ...
        for link in sorted(
            links,
            key=lambda x: not x.get("templated", False)
            if template
            else x.get("templated", False),
        ):
...
class Module(Client):
    ...
    def get(self, dn):
        ...
        for obj in self._search_closed(position=dn, scope="base"):
            return obj.open()
        raise NotFound(
            404, "Wrong object type!?", None
        )  # FIXME: object exists but is of different module. should be fixed on the server.
    ...
    def _search_opened(
        self, filter=None, position=None, scope="sub", hidden=False, superordinate=None
    ):
        ...
        for obj in self._search(filter, position, scope, hidden, superordinate, True):
            yield Object.from_data(
                self.udm, obj
            )  # NOTE: this is missing last-modified, therefore no conditional request is done on modification!
    ...
    def get_layout(self):
        # type: () -> Optional[Any]
        self.load_relations()
        return self.udm.client.resolve_relation(self.relations, "udm:layout").get(
            "layout"
        )
    ...
    def get_properties(self):
        # type: () -> Optional[Any]
        self.load_relations()
        return self.udm.client.resolve_relation(self.relations, "udm:properties").get(
            "properties"
        )
    ...
    def get_property_choices(self, property):
        ...
        return self.udm.client.resolve_relation(
            relations, "udm:property-choices", name=property
        ).get("choices")

    def policy_result(self, policy_module, position, policy=None):
        # type: (str, str, Optional[str]) -> Dict
        self.load_relations()
        policy_result = self.udm.client.resolve_relation(
            self.relations,
            "udm:policy-result",
            name=policy_module,
            template={"position": position, "policy": policy},
        )
    ...
    def get_report_types(self):
        ...
        return [
            x["name"]
            for x in self.udm.client.get_relations(
                self.relations, "udm:report", template={"dn": ""}
            )
            if x.get("name")
        ]
    ...
    def create_report(self, report_type, object_dns):
        ...
        self.load_relations()
        return self.udm.client.resolve_relation(
            self.relations, "udm:report", name=report_type, template={"dn": object_dns}
        )
...
class References:
    ...
    def __getitem__(self, item):
        ...
        return [
            ...
            for x in self.udm.client.get_relations(
                self.obj.hal, f"udm:object/property/reference/{item}"
            )
        ]
...
class Object(Client):
    ...
    def _create(self, reload=True):
        ...
        response = self.client.make_request(
            "POST", uri["href"], data=self.representation, allow_redirects=False
        )
    ...
    def _reload_from_response(self, response, reload):
        ...
        if (
            200 <= response.response.status_code <= 299
            and "Location" in response.response.headers
        ):
    ...
    def _follow_redirection(self, response, reload=True):
        ...
        if (
            response.response.status_code in (201, 202)
            and "Location" in response.response.headers
        ):
        ...
        while (
            300 <= response.response.status_code <= 399
            and "Location" in response.response.headers
        ):
            location = response.response.headers["Location"]
            if response.response.headers.get("Retry-After", "").isdigit():
                time.sleep(
                    min(30, max(0, int(response.response.headers["Retry-After"])))
                )
            response = self.client.make_request("GET", location, allow_redirects=False)
    ...
    def generate_service_specific_password(self, service):
        # type: (str) -> Optional[Any]
        uri = self.client.get_relation(self.hal, "udm:service-specific-password")[
            "href"
        ]
    ...
    def get_properties(self):
        # type: () -> Optional[Any]
        return self.udm.client.resolve_relation(self.hal, "udm:properties").get(
            "properties"
        )
    ...
    def get_property_choices(self, property):
        ...
        return self.udm.client.resolve_relation(
            hal, "udm:property-choices", name=property
        ).get("choices")
