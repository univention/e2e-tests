...
class HomePageLoggedIn(HomePage):
    """
    This represents the logged in state of the
    portal's home page.
    """
    ...
    def set_content(self, *args, **kwargs):
        ...
        self.files_tile = self.page.get_by_role(
            "link",
            name="Files New Tab"
        )
        self.mail_tile = self.page.get_by_role(
            "link",
            name="E-Mail New Tab"
        )
        self.calendar_tile = self.page.get_by_role(
            "link",
            name="Calendar New Tab"
        )
        self.contacts_tile = self.page.get_by_role(
            "link",
            name="Contacts New Tab"
        )
        self.tasks_tile = self.page.get_by_role(
            "link",
            name="Tasks New Tab"
        )
        ...
        self.projects_tile = self.page.get_by_role(
            "link",
            name="Projects New Tab"
        )
        self.activity_tile = self.page.get_by_role(
            "link",
    ...
    def is_displayed(self):
        # TODO: There seems to be nothing that's necessarily
        #  common between the UCS and SouvAP envs
