...
class UDMFixtures:
    ...
    def verify_resource(self, resource_url, data):
        ...
        sent_properties = data["properties"]
        got_properties = result_data["properties"]
        for key, value in sent_properties.items():
            assert got_properties.get(key) == value, (
                f"property {key!r} mismatch: sent={value!r} got={got_properties.get(key)!r}"
            )
