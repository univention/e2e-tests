...
def test_correct_context_csn_after_leader_switch(faker, ldap: LdapDeployment, k8s, k8s_chaos):
...
def test_new_leader_has_correct_context_csn(faker, ldap: LdapDeployment, k8s, k8s_chaos):
    primary = ldap.get_server_for_primary_service()
    conn = primary.connect()
...
def test_write_during_leader_switch(faker, ldap: LdapDeployment, k8s, k8s_chaos):
    primary = ldap.get_server_for_primary_service()
    conn = primary.connect()
